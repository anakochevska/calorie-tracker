  ...
  return (
    <div style={{ position: "relative", width: size, height: size }}>
      <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
        ...
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke={overLimit ? "#EF4444" : color} strokeWidth={stroke}
  ...
  return (
    <div style={{ flex: 1, minWidth: 80 }}>
      ...
      <div style={{ height: 6, borderRadius: 3, background: "var(--ring-bg)", overflow: "hidden" }}>
        <div style={{ height: "100%", borderRadius: 3, background: color, width: `${pct}%`, transition: "width 0.5s cubic-bezier(.4,0,.2,1)" }} />
      </div>
    </div>
  );
};
...
export default function CalorieTracker() {
  ...
  const addEntry = () => {
    ...
    setSelectedFood(null); setManualMode(false); setAiEstimated(false); setAiError("");
  ...
  const showAiButton = foodName.trim().length >= 2 && !selectedFood && !aiEstimated && !cal;

  return (
    <div style={{
      "--bg": "#0F0F0F", "--card": "#1A1A1A", "--card-hover": "#222",
      "--border": "#2A2A2A", "--text-main": "#F0EDE6", "--text-dim": "#7A756D",
      "--accent": MACRO_COLORS.calories, "--ring-bg": "#252220",
      "--font-display": "'DM Serif Display', Georgia, serif",
      "--font-body": "'DM Sans', 'Helvetica Neue', sans-serif",
      "--font-mono": "'JetBrains Mono', 'SF Mono', monospace",
      fontFamily: "var(--font-body)", background: "var(--bg)", color: "var(--text-main)",
      minHeight: "100vh", maxWidth: 480, margin: "0 auto", padding: "24px 16px 80px",
    }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
        <div>
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: 28, fontWeight: 400, margin: 0, letterSpacing: "-0.01em", lineHeight: 1.1 }}>Fuel Log</h1>
          <p style={{ fontSize: 12, color: "var(--text-dim)", marginTop: 4, fontFamily: "var(--font-mono)", letterSpacing: "0.04em" }}>{today.toUpperCase()}</p>
        </div>
        <button onClick={() => setShowSettings(!showSettings)}
          style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 10, padding: "8px 12px", color: "var(--text-dim)", cursor: "pointer", fontSize: 13, fontFamily: "var(--font-mono)", transition: "all 0.2s" }}
      ...
      {/* Settings Panel */}
      {showSettings && (
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, padding: 20, marginBottom: 20, animation: "slideDown 0.25s ease" }}>
          <p style={{ fontSize: 11, fontFamily: "var(--font-mono)", textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--text-dim)", marginBottom: 14, marginTop: 0 }}>Daily Goals</p>
              ...
              <div key={g.key} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "baseline", gap: 3 }}>
                    <input type="number" value={g.value} onChange={e => g.setter(parseInt(e.target.value) || 0)}
                      style={{ background: "transparent", border: "none", borderBottom: "1px solid var(--border)", color: "var(--text-main)", fontSize: 16, fontFamily: "var(--font-body)", width: 60, padding: "2px 0", outline: "none" }}
                    />
      ...
      <div style={{ display: "flex", justifyContent: "center", marginBottom: 24 }}>
        <CircularProgress value={totals.cal} max={dailyLimit} size={190} stroke={10} color={MACRO_COLORS.calories}>
          <span style={{ fontSize: 34, fontFamily: "var(--font-display)", lineHeight: 1, color: remaining < 0 ? "#EF4444" : "var(--text-main)" }}>{Math.round(totals.cal)}</span>
          <span style={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--text-dim)", letterSpacing: "0.1em", marginTop: 4, textTransform: "uppercase" }}>of {dailyLimit} kcal</span>
          <span style={{
      ...
      {/* Add Food */}
      <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 16, padding: 18, marginBottom: 24 }}>
        <p style={{ fontSize: 11, fontFamily: "var(--font-mono)", textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--text-dim)", marginBottom: 14, marginTop: 0 }}>Add Food</p>

        <div style={{ position: "relative", marginBottom: 12 }} ref={suggestRef}>
          <input ref={inputRef} type="text" placeholder="Type any food — e.g. pepperoni calzone..."
            value={foodName}
            onChange={e => {
              setFoodName(e.target.value);
              setShowSuggestions(e.target.value.length > 0);
              if (selectedFood && e.target.value !== selectedFood.name) {
                setSelectedFood(null); setCal(""); setProtein(""); setCarbs(""); setFat(""); setGrams("");
              }
              setAiEstimated(false); setAiError("");
            }}
            ...
            style={{
              ...
              color: "var(--text-main)", fontSize: 14, fontFamily: "var(--font-body)",
          ...
          {showSuggestions && filteredFoods.length > 0 && (
            <div style={{
              ...
              borderRadius: 10, marginTop: 4, overflow: "hidden", boxShadow: "0 12px 40px rgba(0,0,0,0.5)",
            }}>
              {filteredFoods.map(f => (
                <div key={f.name} onClick={() => selectFood(f)}
                  style={{ padding: "10px 14px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", transition: "background 0.15s", borderBottom: "1px solid #252525" }}
                  onMouseEnter={e => e.currentTarget.style.background = "#282828"}
                  onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                >
                  <span style={{ fontSize: 13, fontWeight: 500 }}>{f.name}</span>
                  <span style={{ fontSize: 11, color: "var(--text-dim)", fontFamily: "var(--font-mono)" }}>{f.cal}kcal / {f.servingG}g</span>
                </div>
              ))}
              <div onClick={() => { setShowSuggestions(false); setManualMode(true); setSelectedFood(null); }}
                style={{ padding: "10px 14px", cursor: "pointer", fontSize: 12, color: "var(--text-dim)", fontFamily: "var(--font-mono)", transition: "background 0.15s" }}
                onMouseEnter={e => e.currentTarget.style.background = "#282828"}
                onMouseLeave={e => e.currentTarget.style.background = "transparent"}
              >+ Enter manually</div>
            </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr", gap: 6, marginBottom: 14 }}>
          {[
            ...
            <div key={f.label}>
              <label style={{ fontSize: 9, fontFamily: "var(--font-mono)", textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--text-dim)", display: "block", marginBottom: 4 }}>{f.label}</label>
              <input type="number" placeholder={f.ph} value={f.val}
                onChange={e => f.set(e.target.value)}
                disabled={f.disabled && !manualMode && !aiEstimated}
                onKeyDown={e => e.key === "Enter" && addEntry()}
                style={{
                  width: "100%", boxSizing: "border-box",
                  background: f.disabled && !manualMode && !aiEstimated ? "#0D0D0D" : "#111",
                  border: "1px solid var(--border)", borderRadius: 8, padding: "8px 6px",
                  color: f.disabled && !manualMode && !aiEstimated ? "var(--text-dim)" : "var(--text-main)",
                  fontSize: 13, fontFamily: "var(--font-mono)", outline: "none", textAlign: "center",
                }}
              />
            </div>
          ))}
        ...
        <button onClick={addEntry} disabled={!foodName.trim() || !cal}
          style={{
      ...
      {entries.length > 0 && (
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 12, padding: "0 4px" }}>
            <p style={{ fontSize: 11, fontFamily: "var(--font-mono)", textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--text-dim)", margin: 0 }}>Today's Log</p>
            <span style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--text-dim)" }}>{entries.length} {entries.length === 1 ? "entry" : "entries"}</span>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {entries.map((e, i) => (
              <div key={e.id} style={{
                ...
                animation: i === 0 ? "fadeIn 0.3s ease" : "none", transition: "background 0.15s",
              }}
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                    ...
                    <span style={{ fontSize: 14, fontFamily: "var(--font-mono)", fontWeight: 500, color: "var(--accent)", flexShrink: 0, marginLeft: 8 }}>{Math.round(e.cal)}</span>
                  </div>
                ...
                <button onClick={() => removeEntry(e.id)}
                  style={{ background: "none", border: "none", color: "var(--text-dim)", cursor: "pointer", fontSize: 16, padding: 4, lineHeight: 1, transition: "color 0.15s", flexShrink: 0 }}
      ...
      {entries.length === 0 && (
        <div style={{ textAlign: "center", padding: "40px 20px", color: "var(--text-dim)", fontSize: 13 }}>
          ...
          <p style={{ margin: 0, fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.05em" }}>No entries yet — start logging your meals</p>
        </div>
      )}
